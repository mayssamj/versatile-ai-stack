...
#
# Standalone:  bash mayssam-ai-stack.sh install 20
set -Eeuo pipefail
...
if [[ -z "$TGT" ]]; then
  ...
  err "Add it (from @BotFather for @vz_hermes_controller_bot) then re-run 'mayssam-ai-stack.sh install 20'."
  exit 1
fi
...
[[ "$STATE" == "Ready" ]] || { err "sandbox $SANDBOX not Ready (state='${STATE:-absent}') — run 'mayssam-ai-stack.sh install 04' (OpenShell relay down? 'brew services restart openshell')"; exit 1; }
...
if ! "$OSH" sandbox get "$SANDBOX" 2>/dev/null | sed $'s/\x1b\\[[0-9;]*m//g' | grep -qi 'telegram'; then
  ...
  warn "Re-run 'mayssam-ai-stack.sh install 04' to apply the policy, then re-run this phase."
fi
...
if [[ "$LOCKED" == "1" ]]; then
  ...
  warn "   3. Re-run:        bash mayssam-ai-stack.sh install 20"
