...
if [[ -z "$AT_KEY_CURRENT" ]] || ! printf '%s' "$_at_models" | grep -q '"id"'; then
  ...
  _gen_body="$(python3 -c 'import json; print(json.dumps({"models":["local-gemma4","embed-local","claude-opus-4.8-sub-xhigh","claude-sonnet-4.6-sub-high"],"key_alias":"aitown","metadata":{"owner":"aitown","purpose":"phase36"}}))')"
...
note "Bigger/livelier town (metered): (cd $AT_DIR && npx convex env set LLM_MODEL claude-opus-4.8-sub-xhigh) then restart"
