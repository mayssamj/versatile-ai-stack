...
# The host→backend ADMIN URL the HOST-side `npx convex` CLI dials (env set / env get /
# predev). The override publishes the Convex backend on the LOOPBACK ALIAS $AT_IP (NOT
# 127.0.0.1), so the admin URL MUST be $AT_IP:$AT_BE_PORT — dialing 127.0.0.1:$AT_BE_PORT
# fails with "TypeError: fetch failed" (nothing listens there; admin-key gen dodges this
# only because it uses `docker compose exec` INTO the container). Verified live 2026-06-24.
...
CONVEX_SELF_HOSTED_URL=${AT_CONVEX_URL}
...
perl -e '
  ...
  exit($? >> 8);
' "$AT_PREDEV_TIMEOUT" \
  bash -c 'cd "$1" && env CONVEX_SELF_HOSTED_URL="'"$AT_CONVEX_URL"'" CONVEX_SELF_HOSTED_ADMIN_KEY="$2" npm run predev' \
...
for _ in $(seq 1 30); do
  curl -sL -o /dev/null -w '%{http_code}' --max-time 4 "http://$AT_IP:$AT_FE_HOST_PORT/" 2>/dev/null | grep -q '^200$' && { _fe_up=1; break; }
  sleep 2
done
