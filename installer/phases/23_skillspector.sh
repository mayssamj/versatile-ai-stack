...
#
# Standalone install: `bash install.sh install 23`.
set -Eeuo pipefail
...
if precheck 2>/dev/null && stamp_check "$PHASE"; then
  ok "Phase 23 — SkillSpector — already installed (use 'install.sh install 23' to re-run)"
  exit 0
fi
...
if ! command -v git >/dev/null 2>&1; then
  warn "git not on PATH — cannot clone SkillSpector. Install git (xcode-select --install) then re-run 'install.sh install 23'."
  exit 0
fi
if ! command -v uv >/dev/null 2>&1; then
  warn "uv not on PATH — SkillSpector needs it to build its venv."
  warn "Run 'bash $AI_STACK/install.sh install 14' (Unsloth installs uv), then re-run 'install.sh install 23'."
  exit 0
fi
...
if [[ ! -d "$SS_DIR/.git" ]]; then
  ...
  if ! git clone --depth 1 "$SS_REPO" "$SS_DIR" 2>&1 | tail -3; then
    warn "git clone failed (no network, or the repo moved?). Nothing stamped — re-run 'install.sh install 23' when connectivity is back."
...
if [[ ! -x "$SS_PY" ]]; then
  ...
  if ! uv venv "$SS_VENV" --python 3.12 2>&1 | tail -3; then
    ...
    warn "Resolve the interpreter, then re-run 'install.sh install 23'. Nothing stamped."
    exit 0
  fi
fi
...
if ! uv pip install --python "$SS_PY" -e "$SS_DIR" 2>&1 | tail -8; then
  warn "uv pip install -e failed (network or upstream dep issue). Nothing stamped — re-run 'install.sh install 23'."
  exit 0
fi
...
if ! "$SS_CLI" --help >/dev/null 2>&1; then
  warn "'$SS_CLI --help' did not run cleanly (broken deps?). Nothing stamped — re-run 'install.sh install 23'."
  exit 0
fi
