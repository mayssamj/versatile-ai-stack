...
#
# Standalone install: `bash mayssam-ai-stack.sh install 15`
set -Eeuo pipefail
...
[[ -n "$OSH" ]] || { err "openshell CLI not found — run 'bash mayssam-ai-stack.sh install 04' first."; exit 1; }
...
# model<->agent slugs) so `mayssam-ai-stack.sh model assign/sync` can point Pi at
...
#   bash mayssam-ai-stack.sh install 15
...
note "Models: pi defaults to claude-opus-sub-xhigh (platform default). On-box override: \`pi --model local | local-heavy | local-nemotron3-nano-4b\`"
