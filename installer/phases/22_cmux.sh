...
#     cmux`).
#
# OPT-IN: not wired into the default `install.sh install` run. Add it on demand
# with `bash install.sh install 22`.
#
...
#
# Standalone install: `bash install.sh install 22`.
set -Eeuo pipefail
...
if ! command -v brew >/dev/null 2>&1; then
  ...
  warn "Install it from https://brew.sh then re-run: bash install.sh install $PHASE"
  exit 0
fi
...
if ! brew tap 2>/dev/null | grep -qxi "$CMUX_TAP"; then
  ...
  if ! brew tap "$CMUX_TAP" 2>&1 | tail -5; then
    ...
    warn "then re-run: bash install.sh install $PHASE"
    exit 0
  fi
fi
...
if ! brew install --cask "$CMUX_CASK" 2>&1 | tail -8; then
  ...
  warn "Then re-run: bash install.sh install $PHASE   (it will converge if cmux.app exists)."
...
if ! _cmux_installed; then
  ...
  warn "Re-run after resolving the brew output above: bash install.sh install $PHASE"
  exit 0
fi
