...
# mayssam-ai-stack.sh subcommands so `stack status` and `bash mayssam-ai-stack.sh status` both work.
cat > "$AI_STACK/bin/stack" <<'EOF'
#!/usr/bin/env bash
# stack — daily-driver wrapper around mayssam-ai-stack.sh subcommands.
AI_STACK="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
exec bash "$AI_STACK/mayssam-ai-stack.sh" "$@"
EOF
