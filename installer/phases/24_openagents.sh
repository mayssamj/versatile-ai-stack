...
#   it ONLY when explicitly requested (`mayssam-ai-stack.sh install 24`) and we do NOT
...
#
# Standalone install: `bash mayssam-ai-stack.sh install 24`.
set -Eeuo pipefail
...
if [[ "$OS" != "Darwin" || "$ARCH" != "arm64" ]]; then
  ...
  warn "Skipping (non-fatal). Re-run 'mayssam-ai-stack.sh install 24' on a supported host."
  exit 0
fi
...
if ! curl -fsSL -o "$INSTALLER_TMP" "$OPENAGENTS_INSTALL_URL"; then
  ...
  warn "Skipping (non-fatal) — nothing was installed. Re-run 'mayssam-ai-stack.sh install 24' later."
  exit 0
fi
...
if [[ -z "$AGN" ]]; then
  ...
  warn "  'mayssam-ai-stack.sh install 24' to verify."
  exit 0
fi
