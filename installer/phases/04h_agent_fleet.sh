...
# `mayssam-ai-stack.sh model sync` does, but inline + lock-free — phases run UNDER the
...
# Run the SCRIPT (not `mayssam-ai-stack.sh install 04f`) so we don't re-acquire the lock.
...
if bash "$AI_STACK/installer/phases/04f_hermes_fleet.sh"; then
  ...
  warn "Phase 04f returned non-zero — Hermes fleet may be incomplete (check 'mayssam-ai-stack.sh install 04f')"
fi
...
if sandbox_ready "$PI_SANDBOX"; then
  ...
  warn "$PI_SANDBOX not Ready — skipping Pi upload (run 'mayssam-ai-stack.sh install 15' then re-run this phase)."
fi
...
# widener is `mayssam-ai-stack.sh model sync`, but it re-acquires the install lock (we hold
...
widen_keys() {
  ...
  [[ -n "$master" ]] || { warn "LITELLM_MASTER_KEY absent — skip key widening (run 'mayssam-ai-stack.sh model sync')"; return; }
  ...
  for kenv in HERMES_LITELLM_KEY PI_LITELLM_KEY; do
    ...
    litellm_reconcile_key "$kenv" "$superset" \
      || warn "could not widen $kenv allow-list — run 'mayssam-ai-stack.sh model sync' to finalize"
  done
}
...
note "Canonical finalize (optional): mayssam-ai-stack.sh model sync   (re-renders every agent + re-widens keys)"
