...
if (( HALO_OK )); then
  ...
  if [[ -z "$LITELLM_MASTER_KEY" ]]; then
    ...
    # bin/halo wrapper: route through LiteLLM (local models) by default.
    # HALO's own default model is gpt-5.4-mini (cloud) — but the virtual key is
    # scoped to local models, so we inject a local --model unless the user gives
    # one. base_url + key come from OPENAI_BASE_URL / OPENAI_API_KEY env.
...
[[ -n "\$HALO_BIN" ]] || { echo "halo-engine not installed — run 'bash install.sh install 11'" >&2; exit 1; }
