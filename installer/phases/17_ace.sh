...
#
# Standalone install: `bash mayssam-ai-stack.sh install 17`.
set -Eeuo pipefail
...
if precheck 2>/dev/null && stamp_check "$PHASE"; then
  ok "Phase 17 — ACE — already installed (use 'mayssam-ai-stack.sh install 17' to re-run)"
  exit 0
fi
...
# allowlist-only — `mayssam-ai-stack.sh model list` flags ACE as "(allowlist-only)" so it
...
# Do not edit; re-run 'bash mayssam-ai-stack.sh install 17' to regenerate.
...
AI_STACK="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ACE_DIR="$AI_STACK/ace"
[[ -d "$ACE_DIR/.git" ]] || { echo "ACE not installed — run 'bash mayssam-ai-stack.sh install 17'" >&2; exit 1; }
...
if ! "$ACE_WRAPPER" --help >/dev/null 2>&1; then
  ...
  err "Inspect: $ACE_WRAPPER and re-run 'bash mayssam-ai-stack.sh install 17'."
  exit 1
fi
