...
if [[ -z "$CD_KEY_CURRENT" ]] || ! printf '%s' "$_cd_models" | grep -q '"id"'; then
  ...
  CD_KEY_NEW="$(curl -s --max-time 15 -H "Authorization: Bearer $LITELLM_MASTER_KEY" \
    ...
    -d '{"models":["local-gemma4","claude-opus-4.8-sub-xhigh","claude-sonnet-4.6-sub-high","local-qwen3-coder"],"key_alias":"chatdev","metadata":{"owner":"chatdev","purpose":"phase35"}}' \
...
note "Bigger swarm:      edit a workflow YAML node's name: to claude-opus-4.8-sub-xhigh (metered)"
