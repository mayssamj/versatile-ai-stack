...
# `bash install.sh` (no args) so the user doesn't have to chain subcommands.
#
...
#
# Sourced by install.sh after all phases complete. Must not source anything
...
bootstrap_adopt_foreigns() {
  ...
  for svc in "${foreigns[@]}"; do
    if confirm "Adopt '$svc' now?" Y; then
      ...
      warn "Skipped: $svc remains on old networking. Run 'install.sh adopt $svc' when ready."
    fi
  done
}
...
bootstrap_recreate_honcho_if_drifted() {
  ...
  if confirm "Recreate honcho stack now (compose down/up)?" Y; then
    ...
    warn "Skipped. Run 'install.sh install 03' to migrate honcho when ready."
  fi
}
...
EOF
  ...
  if [[ -z "$pasted" ]]; then
    warn "Skipped. Add PHOENIX_API_KEY=<key> to .env later, then run 'install.sh apply-restarts'."
    return 0
  fi
...
bootstrap_drain_restarts() {
  ...
  if confirm "Apply restarts now?" Y; then
    ...
    warn "Skipped. Run 'install.sh apply-restarts' when ready."
  fi
}
