...
if grep -q 'consult the shared oracle' "$UPG" && grep -A70 'consult the shared oracle' "$UPG" | grep -q 'version_classify'; then ok "check_one catch-all consults the oracle (installed+available, not blanket-manual)"; else bad "check_one still blanket-manual (or the pin/config/owner block displaced version_classify)"; fi
