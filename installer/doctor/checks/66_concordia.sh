...
concordia_diagnose() {
  ...
  [[ -x "$venv/bin/python" ]] || { echo "concordia venv missing ($venv) — re-run 'vz-ai-stack.sh install 37'"; return 1; }
  "$venv/bin/python" -c "import concordia" >/dev/null 2>&1 || { echo "import concordia failed in the venv — re-run 'vz-ai-stack.sh install 37'"; return 1; }
  "$venv/bin/python" -c "import sentence_transformers" >/dev/null 2>&1 || { echo "import sentence_transformers failed (embedder missing) — re-run 'vz-ai-stack.sh install 37'"; return 1; }
  [[ -x "$AI_STACK/bin/concordia" ]] || { echo "bin/concordia wrapper missing — re-run 'vz-ai-stack.sh install 37'"; return 1; }

  local key; key="$(get_env CONCORDIA_LITELLM_KEY '')"
  [[ -n "$key" ]] || { echo "CONCORDIA_LITELLM_KEY missing from .env — re-run 'vz-ai-stack.sh install 37'"; return 1; }
  local models
  ...
  if ! printf '%s' "$models" | grep -q '"id"'; then
    ...
    echo "CONCORDIA_LITELLM_KEY rejected by LiteLLM (no models) — re-mint via 'vz-ai-stack.sh install 37'"
    return 1
  fi
  ...
  echo "Concordia ready (venv + import + embedder + scoped key lists models); prove the sim: vz-ai-stack.sh test 37"
  return 0
}

concordia_fix() {
  echo "vz-ai-stack.sh install 37   # rebuild venv + re-mint scoped key + refresh bin/concordia"
}
