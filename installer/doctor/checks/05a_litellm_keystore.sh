...
# litellm_db_down — SHARED helper (used here AND by the per-phase key checks):
# returns 0 iff LiteLLM is up but its key-store is unreachable (HTTP 503
# `no_db_connection`). Lets a key check say "DB down — heal it, don't re-mint".
litellm_db_down() {
  curl -s --max-time 5 http://litellm:4000/v1/models 2>/dev/null | grep -q 'no_db_connection'
}
