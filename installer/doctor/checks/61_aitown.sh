...
# smoke/36.sh + 'mayssam-ai-stack.sh test 36', not here.
#
...
aitown_diagnose() {
  ...
  [[ -f "$at_dir/docker-compose.yml" ]] || { echo "ai-town/docker-compose.yml missing ($at_dir) — re-run 'mayssam-ai-stack.sh install 36'"; return 1; }
  ...
  if [[ "${running:-0}" -lt 3 ]]; then
    echo "AI Town compose stack not fully up (only ${running:-0}/3 members running) — 'mayssam-ai-stack.sh start aitown' (first build is heavy; 'docker compose -p $project logs')"
    return 1
  fi
  ...
  if ! curl -sL -o /dev/null -w '%{http_code}' --max-time 5 "http://$ip:$fe_port/" 2>/dev/null | grep -q '^200$'; then
    echo "AI Town frontend not serving 200 on http://$ip:$fe_port/ — Vite may still be building, or the stack is down ('mayssam-ai-stack.sh start aitown'; 'docker compose -p $project logs frontend')"
    return 1
  fi

  # Scoped key actually lists models (stale/revoked → 200 + empty data[], so require an "id").
  local key; key="$(get_env AITOWN_LITELLM_KEY '')"
  [[ -n "$key" ]] || { echo "AITOWN_LITELLM_KEY missing from .env — re-run 'mayssam-ai-stack.sh install 36'"; return 1; }
  ...
  if ! printf '%s' "$models" | grep -q '"id"'; then
    ...
    echo "AITOWN_LITELLM_KEY rejected by LiteLLM (no models) — re-mint via 'mayssam-ai-stack.sh install 36'"
    return 1
  fi
  ...
  echo "AI Town ready (3/3 containers up + frontend 200 + scoped key lists models); watch it: http://aitown:$fe_port/ — prove the wiring: mayssam-ai-stack.sh test 36"
  return 0
}

aitown_fix() {
  echo "mayssam-ai-stack.sh start aitown    # bring the compose stack up (first build is heavy)"
  echo "mayssam-ai-stack.sh install 36      # rebuild override + re-mint scoped key + re-wire Convex LLM env + re-push schema"
}
