...
# lo0 aliases, this check fires on the next `mayssam-ai-stack.sh doctor` run.
...
container_alias_routable_diagnose() {
  ...
  if (( ${#offenders[@]} > 0 )); then
    ...
    printf '  - %s\n' "${offenders[@]}"
    echo "(this almost always means lo0 aliases regressed; run mayssam-ai-stack.sh prepare-sudo)"
    return 1
  fi
}
...
container_alias_routable_fix() {
  ...
  warn "Run:  sudo bash $AI_STACK/mayssam-ai-stack.sh prepare-sudo"
