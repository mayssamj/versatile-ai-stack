...
chatdev_diagnose() {
  ...
  # SILENT-403s the model the sim calls. Resolve the bound model the way phase 35 does
...
print("__wildcard__" if (not m or any(x in ("all-proxy-models","all-team-models") for x in m)) else "\n".join(m))' 2>/dev/null || true)"
    if [[ -n "$allow" ]] && ! printf '%s\n' "$allow" | grep -qxF '__wildcard__' \
      ...
      echo "CHATDEV_LITELLM_KEY allow-list missing '$want' (the model ChatDev calls) — stale key after a model rename/re-assign; re-run 'vz-ai-stack.sh install 35' to self-heal"
      return 1
    fi
  fi
