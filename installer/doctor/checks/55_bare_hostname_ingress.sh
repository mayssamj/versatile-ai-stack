...
bare_hostname_ingress_diagnose() {
  ...
  if ! launchctl print "system/com.ai-stack.ingress" >/dev/null 2>&1; then
    echo "caddy installed but ingress daemon not loaded — run 'vz-ai-stack.sh ingress up'. [skip]"
    return 0
  fi
  ...
  if [[ "$lit" != "127.0.10.1" || "$phx" != "127.0.10.2" ]]; then
    echo "ingress :80 not isolated per-IP (litellm->${lit:-unreachable}, phoenix->${phx:-unreachable}; expected 127.0.10.1 / 127.0.10.2). Re-run 'vz-ai-stack.sh ingress up'."
    return 1
  fi
...
bare_hostname_ingress_fix() {
  ...
  warn "    bash $AI_STACK/vz-ai-stack.sh ingress up"
  warn "    bash $AI_STACK/vz-ai-stack.sh ingress trust   # trust the local CA for https://"
  return 1
}
