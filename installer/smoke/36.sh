...
# 4. the backend is actually WIRED (container env OR convex env get == host.docker.internal:4000)
...
if [[ "$_be_env" == "$AT_CONVEX_LLM_URL" ]]; then
  ...
  if [[ -n "$_admin" ]] && command -v npx >/dev/null 2>&1; then
    ...
    warn "could not confirm Convex LLM_API_URL (no AITOWN_ADMIN_KEY or npx) — chat path (step 3) verified, but wiring-into-the-town is UNVERIFIED. Confirm: (cd $AT_DIR && npx convex env get LLM_API_URL)"
  fi
fi
