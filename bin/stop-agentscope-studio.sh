#!/usr/bin/env bash
# stop-agentscope-studio.sh — `vz-ai-stack.sh stop agentscope-studio` entrypoint.
...
# re-enable the GUI with `vz-ai-stack.sh start agentscope-studio`. Mirrors stop-aionui.sh.
set -Eeuo pipefail
