...
if [[ -f "$PID_FILE" ]]; then
  ...
  if pid_is_ours "$pid" && http_ok "http://$HOST:$PORT/"; then
...
log "Starting claw3d (dev) on http://$HOST:$PORT (first boot compiles; can take 30-90s)..."
( cd "$CLAW_DIR"
  PORT="$PORT" HOST="$HOST" nohup node server/index.js --dev >> "$LOG_FILE" 2>&1 &
  echo $! > "$PID_FILE"
) </dev/null
sleep 2
pid="$(cat "$PID_FILE" 2>/dev/null || echo "")"
...
i=0; while (( i < 180 )); do
  port_listening "$PORT" && http_ok "http://$HOST:$PORT/" && { ok "claw3d running (pid $pid) → open http://localhost:$PORT"; exit 0; }
  sleep 1; i=$((i+1))
done
