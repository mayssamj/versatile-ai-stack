...
- **Propagate the constitution** — every subagent I spawn gets these rules and is held to them; I keep them aligned and prevent drift.
...
# mcp_servers: enumerate the wired servers (github / slack / data); reads free, send/post/mutate = §5
```
```bash
...
> **Note.** On Hermes I'm the only router — routing is realized by the fleet harness (the kanban board + dispatcher across the 9 profiles), not a Task primitive. I own the global turn budget and execute directly when fastest; my own edits still pass the gates.
