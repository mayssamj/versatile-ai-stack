...
```bash
hermes profile create incident-manager
hermes -p incident-manager ...                 # run a one-off command as this profile
...
# mcp_servers: add status-page / pagerduty / slack etc. as needed
```
```bash
# install this profile's skills (shared discipline + team-protocol)
hermes -p incident-manager skills install official/<category>/<skill>   # repeat per skill
hermes -p incident-manager skills inspect                                # verify exact tool IDs for your version
```

> **Note.** Coordinator profile — read-only on systems plus comms. Gate any broadcast or production action with `disable-model-invocation: true`. Activates out-of-band; it is not part of the build pipeline.
