...
3. **Heals** broken installs via `bash install.sh doctor` — detects drift between declared and actual state, diagnoses the failure, applies the fix.
...
- `install.sh` (no args) → interactive top-to-bottom install with a phase menu, resumes from last incomplete phase
- `install.sh install <phase>` → install a specific phase (e.g. `01h` for Phoenix)
- `install.sh doctor` → full health check + auto-fix; reports what was fixed
- `install.sh doctor <service>` → diagnose one service
- `install.sh status` → tabular service status (declared vs actual, like `kubectl get pods` but for this stack)
- `install.sh logs <service>` → tail recent logs from a service (avoids the user remembering `docker logs <name>`)
- `install.sh reset --confirm` → DANGEROUS, wipes all state, requires explicit confirmation
- `install.sh test <phase>` → run the validation suite for a phase without reinstalling
...
```
~/ai-stack/
├── install.sh                    # entry point, dispatches subcommands
├── installer/
│   ├── lib/
...
4. The suggested next step (run `install.sh doctor <service>`, edit a specific file, etc.)
5. How to resume after fixing (`install.sh install <phase>` will resume from this phase)
...
7. **Before declaring "done"**: actually run `bash install.sh` end-to-end in your sandbox (or as close as you can get) and verify it works for at least the first 3 phases. Report what you couldn't verify (sandbox limitations) so Mayssam knows where to be careful.
