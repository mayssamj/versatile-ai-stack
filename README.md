...
- **One installer, self-healing and reversible** — brings up all 40 services, resumes if interrupted, never destroys a running container without confirmation, and proves itself with 48/48 doctor checks.
...
- **Something's broken** — read [DOCTOR.md](doc/DOCTOR.md) for what each of the 48
...
│   ├── doctor/checks/      # one file per failure mode (48 checks)
...
- **28 core install phases (+6 opt-in extras: portless · cmux · skillspector · openagents · lmstudio · mempalace) · 40 services · 48 doctor checks.**
