...
- **`doc/updated-SOUL.md` — a PROPOSED successor to the 25-rule constitution (not yet adopted; `doc/SOUL.md` remains the live `@`-import).** Produced by a §24 council run at full scale: 3 independent reviewers (adversarial · AI/agentic-systems · prompt-engineering, all read-only `Explore` agents so they could not edit) → 42 findings / 20 proposed additions → a cross-debate round (23 rebuttals, 14 unresolved) → a neutral synthesist (21 agreed changes, 16 rejections, 3 escalations) → a ratification round against the written draft (3× `ship-with-fixes`, 21 must-fix, 7 false-claim findings). 1299 → 2368 words. **Structure:** an unnumbered `§P` precedence block (clauses labelled `§P.a`–`§P.d` so they cannot be mistaken for Rules 1–4), then **Rule 26 — the hard line** printed above Rule 1 and flagged as the only out-of-sequence number, then a dated host-limit fence, then Rules 1–25 ascending. **Rules 1–25 keep their numbers and meanings** (`§24 council` is cited 77× across this repo, plus `SOUL §5`/`§8`/`§21`/`§25`); 7, 11 and 23 are absorbed into 6, 9 and 21 with no pointer stubs, each named in the absorbing rule's heading — verified: `SOUL #7`, `#11`, `#20`, `#23`, `#12`, `#17` are cited **zero** times anywhere.
  **What the always-loaded constitution was missing:** (1) **any hard line at all** — its only prohibition was Rule 24.1's undefined "unless a real destructive action", while the enumerated never-list lived only in `fleet/manager.md` and the `team-protocol` skill, so an agent handed "the constitution" per Rule 21 inherited maximal autonomy with zero named prohibitions — the gap both unrecoverable losses on record happened inside (a live gitignored `.env`; a dispatched verifier writing the real `~/.npmrc`); (2) **a precedence clause** — 25 rules, ≥4 live conflicts, no arbitration, so collisions resolved by whichever rule the model weighted last; (3) **a size trigger on §24.2** — "everything you do … 3 reviewer agents" is unsized, fights 24.1, and `CHANGELOG.md:1796` records the tension as knowingly shipped unresolved; (4) **the delegation mechanism** Rule 21 asserts but never specifies (read-only enforced by *omitting* Edit/Write, disjoint write paths, throwaway HOME, verify the artifact landed) — the gap `CHANGELOG.md:39` already flagged as "SOUL #21 dropped exactly where it matters"; (5) **a landing step** — pull/commit/integrate/push was absent from the definition of done.
  **What was counterfactual and is now repaired:** Rule 5's "browser on Windows PC" and Rule 9's "WSL2 / Docker bind mount / Cowork mount" (this host is macOS/Apple Silicon — no Windows, no WSL2); Rule 22's "every 5 minutes" (a turn-based agent has no wall clock) → event-triggered; Rule 20's "memorize" (no such primitive) → number repurposed to *persist the correction to the memory surface*; Rule 10's "No exceptions" (broken by this repo's own `TROUBLESHOOTING-PROMPT.md`, which prescribes `docker ps --format 'table {{…}}'`); Rule 25 bullet 2's two invented facts — naming `doctor` as worktree-guarded (`installer/lib/worktree.sh:15-18` documents read-only ops as deliberately unguarded) and `recreate` as a subcommand (it is a flag) — i.e. the constitution was violating its own Rule 15. Verified against source: `worktree_guard` fires on install/adopt/gc/upgrade/start/stop/apply-restarts/reset, plus `worktree_guard_soft` on doctor's auto-heal.
  **Three escalations ruled by the orchestrator:** zero-reviewer tier = **no panel only when nothing changes and no finding is asserted**, ≥2 the moment a byte changes (the operator's own written floor at `doc/ONBOARDING-PROMPT.md:49`; the self-graded "trivial" tier was refused) **+ 24.4's debate round now fires only where reviewers disagree**, which is what makes the ≥2 floor affordable on a one-line change; machine facts = **split**, the prohibition stays in SOUL.md so it reaches a dispatched subagent while descriptive hardware facts belong in `~/.claude/CLAUDE.md` (prohibitions do not rot, descriptions do — Rules 5/9/25 are the proof); an operator instruction **may** waive a verification rule but only when he names the check, and the agent must then report the hole — overruling the synthesist's "never waivable", because the operator's own canon already forbids a verification action (opt-in `*_DEEP_CHECK` model cold-loads), and an absolute the canon itself breaks is what teaches that numbered rules are negotiable.
  **Defects the ratification round caught in the draft, all fixed:** Rule 26's enumeration had drifted to "config you did not create" — a self-graded exemption on the file's supreme rule — while the next sentence claimed sameness with `team-protocol` §5's "deleting important work"; `§P.b` listed Rule 25 as "process ceremony" and `§P.c` makes ceremony waivable, which silently re-created the ALWAYS-worktree exception the council had explicitly rejected; `§P.c` had dropped the word "explicitly", landing the waiver between the ruled option and the one it replaced; the host fence pointed at `~/.claude/CLAUDE.md` for facts that are verifiably not there (it is 4 lines / 193 bytes of `@`-imports, one of them SOUL.md itself — a circular pointer); Rule 8's "outside the repo" excluded `.env`, which is *inside* the repo (`.gitignore:2`) and is the one file whose loss was unrecoverable; and Rule 18's unprompted `push` collided with Rule 26's "publishing externally" (now explicitly not a Rule 26 event). **A second stale copy of the constitution was also found** at the gitignored `ingestor/processed/ai-stack-doc/SOUL.md` — inside the RAG corpus, still carrying the "Windows PC" line, and invisible to this shell's gitignore-aware `grep` (the same trap that made all three round-1 reviewers report different citation counts, which is why "re-verify your own synthesis, not just the reviewers" is now in 24.4).
  **Adoption is a separate change** — nothing is live until `doc/updated-SOUL.md` replaces `doc/SOUL.md`. That change must, in the same commit, drop the count from the 10 tracked "25-rule"/"25 rules" claims (`doc/ONBOARDING-PROMPT.md:22`, `doc/AGENT-ONBOARDING.md:73` and `:527`, `doc/TROUBLESHOOTING-PROMPT.md:79`, `doc/specs/2026-06-11-manager-second-brain.md:6` and `:76`, `fleet/manager.md:70`, `agent-profiles/pi/agents/manager/SYSTEM.md:73`, `agent-profiles/hermes/profiles/manager/SOUL.md:73`) — `fleet/manager.md` most urgently, since it is `@`-imported alongside SOUL.md and would tell every session "25 rules" while the loaded file has 26 — and re-run the ingestor so the corpus copy is re-derived rather than hand-edited. Per this repo's own standing rule, ship no counts: "a number in a shipped doc is a fact with an expiry date."
...
- **Project + CLI renamed (operator request): GitHub repo `versatile-ai-stack` → `mayssam-versatile-ai-stack`, entrypoint `vz-ai-stack.sh` → `mayssam-ai-stack.sh`.** `git mv` + one idempotent sweep over every tracked mention — **280 files**: all `bin/` launchers, every installer phase/check/smoke/test, `services.yml` help prose, agent-profile SOULs, docs, prompts, README (TUTORIAL/DIAGRAMS HTML regenerated via their builders, `--check` green). Token accounting reconciled exactly: 2,418 `.sh` references before = 2,418 after, 0 old tokens remaining (case-insensitive). Hand-fixed what no literal sweep can see: the tutorial nav logo was `vz<span>-ai-stack</span>` — the brand token split across HTML tags — now the house `ai-stack` mark matching DIAGRAMS/USER-GUIDE; README H1, tutorial title, and TROUBLESHOOTING-PROMPT intro carry the PROJECT name (doc titles = project name, command examples = script name). Functional renames ride along: the openshell-watchdog live-install identity grep (`ps … | grep 'mayssam-ai-stack'` — a running watchdog must be restarted to pick it up) and phase 27's default `SOURCEGRAPH_INDEX_REPOS=mayssamj/mayssam-versatile-ai-stack` + `smoke/27.sh`'s `repo:` query — the live Sourcegraph external-service config needs a re-sync after the GitHub rename (old clone/URL forms keep working via GitHub's automatic redirect, so nothing breaks in the interim). Deliberate exceptions, on the record: internal `_vz_*` shell identifiers (`_vz_bounded`, `_vz_args`) are not the script name — renaming live function internals is regression risk with zero user-visible benefit; `@vz_hermes_controller_bot` is a real external Telegram bot handle; `doc/ALTERNATIVES.md`'s "versatile experimental" is the English word. Rollback: `git revert` of this commit + `gh repo rename versatile-ai-stack`.

### Features

- **`doc/TROUBLESHOOTING-PROMPT.md` — a paste-ready fix-it prompt an END USER hands to their own coding agent**, closing the last gap in the doc-role taxonomy: `ONBOARDING.md` = end-user usage, `ONBOARDING-PROMPT.md` = bootstrap a new *owner*, `TROUBLESHOOTING.md` = human recovery recipes — and nothing for "I hit a bug and want an agent to fix my install". The user copies the body below the `---` as their agent's first message and describes the problem; the agent diagnoses and repairs. Carries §1 hard limits (blast-radius-first, because unlike the owner prompt this agent runs on a *stranger's* machine with their `.env`, containers, and uncommitted work), a 12-rule distillation of `doc/SOUL.md` keeping the original numbering, ground-truth commands, an 8-layer triage table, the codebase's real footguns, a definition of done, and a report shape that ends in a copy-pasteable upstream issue report. Scoped to cover change requests too (§6), since half of "I want X to work differently" is really "X is misconfigured" — one file, because the user can't reliably classify their own problem up front. **§24 council (fact-check + adversarial + architect + qa + PM; 71 findings → 35 adversarially verified → 20 upheld) caught six defects in the first draft, all of the same class — the doc named commands that return a GREEN line while doing nothing:** (1) it never warned that a bare `mayssam-ai-stack.sh` *is* `install all` (`main()`'s `local cmd="${1:-install}"` + the `install|"")` arm) — a minute-one agent move that would have bootstrapped host packages, rewritten the `.env` baseline, and re-run every phase *over the evidence of the bug*; (2) its prescribed fix for the commonest symptom class was a no-op — most phases open with a completion gate and `exit 0`, and no gate reads a *value* from `.env`, while a bare start script prints `✓ already running (use --recreate to rebuild)` — so §1 ("never pull a model") and §5 ("re-running a phase is safe") actively contradicted each other, since the inference and lumen phases `ollama pull`; the real verb is `bin/start-<svc>.sh --recreate`, proven by a moved `docker inspect -f '{{.Created}}'`; (3) it guarded `.env` but left `docker inspect` wide open — `bin/start-litellm.sh:229-235` passes seven real keys as `-e`, and §4's own "I changed it and nothing happened" row is what sends an agent there (now: print env *names* only, hand over log *paths*, never contents); (4) two affirmatively wrong paths — `log/` is agent-simulation output, not stack logs (`installer/state/install-latest.log` is the install transcript), and `logs <svc>` is a raw `docker logs` that fails for the many host-process services, which a literal-minded agent reads as "the service doesn't exist"; (5) §7 spawned reviewers with fresh context and gave them neither §1 nor read-only — SOUL #21 dropped exactly where it matters, since a child told "prove it works" can run `install all` (*same omission still open in `doc/ONBOARDING-PROMPT.md:49-53` — filed, not fixed here*); (6) the worktree bullet was maintainer doctrine in a user's file, and wrong twice — it named `doctor` as guarded (it isn't; `installer/lib/worktree.sh:15-17` documents read-only ops as deliberately unguarded — the 8 `worktree_guard` callsites are install/adopt/gc/upgrade/start/stop/apply-restarts/reset) and `recreate` as a subcommand (it's a flag), inventing a flag in a doc whose own §15 forbids inventing flags. Re-verifying the council's own output before applying it caught three transcription errors in its synthesis (28 of 49 phases carry the gate, not 47; 10 of 29 start scripts take `--recreate`, not 16; `log/` is 588 files of which 441 are *not* `oasis-*`) — **all counts were dropped from the shipped file rather than corrected**, same reasoning that removed the service count: a number in a shipped doc is a fact with an expiry date, and the fleet already disagrees with itself (README 53 · HANDOFF 51 · ONBOARDING-PROMPT ~51). Ten council proposals were rejected on verification, notably: banning `stack test 01` (refuted on magnitude — `local` is nemotron-3-nano:4b and the pings are `max_tokens:1` — and on coherence, since `TROUBLESHOOTING.md:885-890` lists it as a routine step, so the ban would contradict a doc the prompt orders the agent to read); prescribing `NO_PROMPT=1` for the baseline doctor run (actively harmful — `doctor.sh:134` is evaluated *before* the `AUTOHEAL` branch at `:136`, so it suppresses the self-healing checks including the documented common-503 root cause); documenting the non-TTY guard's "bypasses" (3 of 4 don't work — `yes |` makes stdin a pipe, so the guard fires *harder* — and publishing them hands a stranger's agent two env-var names it otherwise never learns); and a baseline-relative definition-of-done (which formalizes the exact failure its proposers feared: *"your stack had 6 reds, it still has 6 reds, done"*).
- **Nothing in the installer used to name a recovery doc — now the two moments of maximum frustration do.** New shared `common.sh::print_repair_hint` (the `print_inference_hint` convention: one body, one lib, both callers already source it) emits the pointer to **stderr** with **absolute** `$AI_STACK/…` paths — `bin/stack` is PATH-able and `bash ~/ai-stack/mayssam-ai-stack.sh` is the documented form, so a repo-relative path is exactly wrong from any other cwd, at the one moment the message assumes the reader is already stuck. Called from `installer/doctor/doctor.sh` when checks stay red, and from `mayssam-ai-stack.sh::install_on_exit` (the post-lock EXIT trap — the one seam where all three `run_phase` failure paths converge) with an extra line naming the run transcript; the recorded rc is returned unchanged. A green doctor and a successful install print nothing. **§24 council (adversarial + architect + qa, all three approve-with-changes) found five things worth fixing and one thing that was already right.** Fixed: (1) **the prompt told users' agents to leak their own API keys** — `cat .env > .env.bak.$(date +%s)` creates the copy under the shell's umask (0644 on macOS) from a 0600 source, inside the bullet titled "it holds my real API keys"; now wrapped `( umask 077; … )`, matching the repo's own idiom at `mayssam-ai-stack.sh:546`; reproduced both modes before/after. (2) **`verify` printed the hint four times** — `cmd_verify` shells out to `doctor.sh` four times with single-check filters, on precisely the broken-host state it exists to detect, so an ungated hint buried verify's own verdict under 12 lines; now gated on an unfiltered run (`[[ -z "$FILTER" ]]`), since `doctor <check>` is a targeted probe, not the "I'm stuck" moment. (3) **Ctrl-C was reported as "Stuck? Hand it to a coding agent"** — a deliberate abort the installer itself advertises (`"Ctrl-C now to abort"`), against a repo convention that already maps signal-stops to clean (`run_foreground_server`'s 130/143). Sniffing `_rc` for 130 does **not** work — the `install all` loop rewrites a dead phase to `exit 1`, so the signal is invisible by then — hence a sentinel set by a re-declared `INT`/`TERM` trap. (4) **the two hints had drifted in wording *and* stream** within a single change (stdout vs stderr, "describe the problem" vs "describe what failed"), and the install one never named `TROUBLESHOOTING.md` while the doctor one never named the transcript — collapsed into the shared helper. (5) the seam comment claimed "the ONE place every failure path passes through", which is false: the trap is installed after `worktree_guard`/`install_log_start`/`preflight`/`lock_acquire`, so a sudo refusal or host-dep bootstrap failure exits before it — comment corrected rather than moving the trap, because `install_on_exit` calls `lock_release` and trapping earlier would let a preflight failure delete another process's lock. What the adversarial lens could **not** break, and confirmed empirically: `(( _rc != 0 ))` sits in an `if` condition, exempt from errexit and the ERR trap, so no spurious `✗ ERR`; rc 0/1/2/130 all survive; and the printfs land *after* `install_log_drain` restores fd 1/2 off the tee, so the hint can neither be double-captured into the log nor race tee's flush — ordering that is correct, not lucky.
- **`mayssam-ai-stack.sh logs <container>` never worked in its documented one-argument form** (found by the council's qa lens while fact-checking the new prompt, which points agents at it). `cmd_logs` was `run_foreground_server docker logs "$1" "${2:-}"` — the quoted `"${2:-}"` always handed docker an empty second argument, so a bare `logs litellm` died with `docker: 'docker logs' requires 1 argument`. Now shifts and forwards `"$@"`, so both `logs <ctr>` and `logs <ctr> --tail 200 -f` work; verified live before/after. (**Filed, deliberately not fixed here:** `logs` is the only diagnostic verb not routed through `diag_exit`, so a bad container name also prints a spurious `✗ ERR line …` — reproduced on unmodified `main`. It's a 2-line fix, but `diag_exit` wraps a *blocking* foreground server and the Ctrl-C path can't be verified headlessly; unverifiable signal-handling changes don't ride along in a docs change.)
- **Regression pins, in the harness that already existed.** `installer/smoke/install-log.sh` extracts `install_on_exit` verbatim and drove it at `MODE=onexit BODY_RC=7` while discarding output — so the hint could have been deleted with every assertion still green. Added a non-vacuity guard (the extracted block must contain the `print_repair_hint` call) plus **4c/4d**: a failed install prints the hint with an absolute path *and* the transcript; a successful one stays silent. Both **mutation-tested** — deleting the call fails assertion 0, making it fire unconditionally fails 4d, restore is green. Battery after the change: `install-log` smoke + `doctor_err_trap_exit` 7 · `doctor_noninteractive_guard` 14 · `no_glued_multibyte_var` 2 · `install_doctor_determinism` 42 · `takeover_hardening` 59 (runs the REAL `doctor.sh` against a synthetic fixture) · `foreground_server_signal_exit` 6 — all 0 failed. Docs swept in the same change (per the doc-sweep discipline): README (quickstart + the "Something's broken" ladder), `ONBOARDING`, `USER-GUIDE`, `TUTORIAL` (prose + reference table, **+ HTML regen** — check 45 diffs the generated page against a fresh build; `--check` confirmed drift before / sync after), `INSTALL`, `OPERATIONS`, `DOCTOR` (the new output contract, beside the exit-code table), `TROUBLESHOOTING` (header + the "diagnosing from scratch" dead-end), `HANDOFF` (the doc-role taxonomy), `AGENT-ONBOARDING` (so the owner maintains it and doesn't duplicate it). Also corrected in passing, since it sat in a paragraph this change already touched: README's "Something's broken" ladder said **68** doctor checks against a badge (and a live run) of **84**.

## 2026-07-21

### Fixes

- clean-install→doctor determinism (§24 council; operator hit 3 doctor reds right after `reset hard` + `install all`): **(1) pipefail-EPIPE flake class closed** — `litellm_has_callback` was `yq | grep -qxF`, and yq line-flushes, so grep -q's early exit at the mid-list `arize_phoenix` match SIGPIPE'd yq (rc 141) → pipefail read a CORRECT config as "callback missing" ~40% of runs (check 06 red on a green install; reproduced 2/5 live). Converted 17 racy sites across three producer classes: capture-then-grep for yq/curl-body/`lms_served_ids`/`sandbox list` membership tests (litellm.sh ×3, fleet.sh, lmstudio.sh, 25_lmstudio), `awk END`-fold with no early exit for awk-mid-pipe (deps.sh + status.sh + 01_inference brew-services — a flaky "not started" there triggered a spurious `brew services start ollama`, the known OLLAMA_HOST-wiping path; 16_lumen ×2, verify.sh lo0, bin/pi + bin/pi-as Ready gates), and `grep -c` count-then-judge for streaming `docker logs` scans (checks 06/09/13 — the -q form also false-GREENed real OTLP/401 errors when the producer lost the race). Buffered single-write producers (`docker ps` tabwriter, printf builtins, trailing `curl -w` codes) deliberately untouched. NEW advisory doctor check 83 `pipefail_grep_epipe_guard` (static, single-line heuristic, self/comment/inner-shell excluded, print-only fix) — count 83→84, docs swept across DOCTOR (+§83) / ARCHITECTURE / INSTALL (81/84) / ONBOARDING / AGENT-ONBOARDING / TROUBLESHOOTING / TUTORIAL (+HTML regen). **(2) paperclip red closed end-to-end** — a harness group-SIGTERM killed the daemonized `pnpm dev` (installer's process group), `cleanup` had gutted `node_modules` (tsx, the dev entrypoint, unresolvable), and phase 08's dir-existence gate (`[[ -d node_modules ]]`) skipped the repair forever while `stamp_mark` ran unconditionally — install "complete", doctor red, no re-run path. Phase 08 now gates `pnpm install` on the tsx entrypoint (integrity, not existence), stamps ONLY after a healthy `precheck` (not-up → loud warn, NO stamp, soft exit 0 so `install all` continues past a broken best-effort agent), and precheck gained the cwd identity fallback (the `pnpm dev` argv carries no clone path — verified live; without it a HEALTHY daemon could never re-stamp). start-paperclip.sh self-heals a gutted tree pre-launch (same tsx gate) and launches app + relay in their OWN process groups (`set -m` subshell, macOS-verified) so an installer-group SIGTERM can't reap them again. **(3) hermes Slack role router now supervised (W2b)** — the router is an in-sandbox process with a launcher but had NO relaunch path (the 2026-07-21 OrbStack engine restart left it dark ~17h while W2 self-healed the Telegram gateway); openshell-watchdog gains W2b under the same knob/gates as W2: relaunch ONLY on a STALE pid file (present + process dead = crash/reboot; ABSENT = never configured or deliberately stopped — operator intent never overridden), via the persisted phase-38 launcher (`HOME=/sandbox`, docker exec not the relay, bounded, notify on failure). Also: check 17's alias probe no longer prints `HTTP 000000` (curl's own `000` concatenated with the `|| echo 000` fallback — fallback moved to a separate assignment + 3-digit shape guard, the check-40 idiom). New offline suite `test_install_doctor_determinism.sh` (41 asserts: every conversion pinned + check-83 behavioral drives incl. a piped-filter fixture + REAL `litellm_has_callback` 50× under pipefail); full battery 28/28 suites green. §24 council round (adversarial approve · architect + qa approve-with-changes → consensus): the architect caught that the sweep's and check 83's shared `[^|]*` regex was blind to PIPED yq filters — a live racy site survived at 25_lmstudio.sh:159 (assigned-MLX loads skipped ~40% of runs when an MLX slug is assigned); converted it, widened the guard to `.*` (tree still scans clean — no false positives), and seeded the blind-spot fixture; both other reviewers demanded an identity leg on W2b's alive-check (bare kill -0 masks a recycled PID after an engine restart — /sandbox persists, the PID namespace resets) — the probe now confirms /proc/<pid>/cmdline matches the router before trusting the pidfile. Follow-up (same arc, live-caught twice, root-caused 2026-07-22): the recurring "Slack dependencies are missing" was NOT lost deps — slack_bolt sat importable in /sandbox/.venv the whole time; the router launcher ran bare python3, which resolves to the uv SYSTEM python in any non-login shell (watchdog docker-exec relaunches, post-restart), where the venv site-packages are invisible — only openshell relay sessions source the profile that puts .venv/bin first, which is why `install 38` always "healed" it. Launcher now uses the absolute /sandbox/.venv/bin/python3 (the W2 gateway idiom), and the watchdog W2b relaunch runs `docker exec --user sandbox` — a root-run router (docker exec default) poisons /sandbox artifacts root-owned and relay sessions (uid 998) get EPERM from kill -0 on it, so check 67 reads a HEALTHY router as "not running"; both proven live through the exact previously-failing paths (relay probe YES, Socket Mode reconnected as the sandbox user). Belt on top: handle_storm re-runs phase 38 in the hermes heal, gated on BOTH HERMES_SLACK_{BOT,APP}_TOKEN exactly like Telegram gates phase 20, and the 3 manual-heal doc snippets now include 38.
...
- anonymous-volume leak closed (§24 council: adversarial+architect+qa, 20 orphans ≈1.5GB found after the first `reset hard`): every ai-stack `docker rm -f` sink now passes `-v` (reset managed+compose sweeps, docker.sh `recreate_guard` — the highest-traffic path, gc.sh reaper, chatdev recreate/uninstall) so a container's ANONYMOUS volumes (chatdev/ai-town node_modules mask-guards — anonymous BY DESIGN per b5e4d88, never name them; image-VOLUME scratch) die with it — named volumes/binds are untouched by `-v`; `reset hard/nuke` adds a DIFF-SCOPED sweep (entry snapshot → removes only what THIS run orphaned, e.g. gateway-owned sandbox homes that can't take rm -v; each tar-backed-up fail-closed; pre-existing/foreign orphans NEVER touched) + `aitown` joins the compose teardown list; `cleanup --docker` now itemizes dangling anonymous volumes (size via `docker system df -v`, age) and `--yes` tar-backs-up each to `data/volume-backups/cleanup-<ts>/` before removing (non-recoverable ≠ image prune); NEW advisory doctor check 82 (census ≤5, skip-clean on docker-down, print-only fix → cleanup) — count 82→83; shared verb `docker_anon_orphans` in docker.sh (single implementation, three callers); FLEET-DURABILITY documents the checkpoint scope honestly (`docker commit` = writable layer only; /sandbox IS captured+restorable, mounted volumes are NOT — nothing durable there today, audited); TUTORIAL hard-reset bullet corrected (hermes-workspace/aitown were omitted); new offline suite test_reset_anon_volumes.sh (extract-real-fn + docker() stub: diff-scoping, fail-closed backup, FORCE_WIPE override, -fv wiring statics)
- test_doctor_noninteractive_guard.sh un-drifted (was 8/1 on main; test-only, production doctor unaffected): the FIX_CAPABLE round added a new first guard to doctor.sh's fix-dispatch block, but the test's extract-the-real-block harness never declared `FIX_CAPABLE` — under `set -u` an UNDECLARED assoc array is treated as indexed, so the subscript `fake` was arithmetic-evaluated → `fake: unbound variable` crashed every drive; the AUTOHEAL case failed honestly while both skip-expecting cases passed VACUOUSLY (green because of the crash). Now 14/0: harness declares both arrays behind a bash-4+ self-gate (stock bash 3.2 previously aborted mid-run yet exited 0 — another vacuous pass); prompted-path cases carry `FIX_CAPABLE=([fake]=1)` (unmarked would land in the advice-only branch, which RUNS the body); new integration + matrix cases pin the advice-only contract with REAL-code drives in both NO_PROMPT modes (unmarked body runs in EVERY mode, print-only by author assertion) and pin NO_PROMPT-beats-AUTOHEAL; every drive asserts EMPTY captured stderr so a future harness/block crash fails loudly instead of passing vacuously (both mutation classes proven red); the structural ordering check now looks up each branch's line individually — the old combined-grep assertion was a tautology (grep emits in file order) and its labels had silently drifted
...
- **Stop `bin/halo` + `litellm/config.yaml` from re-dirtying git on every install (tracked generated artifacts).** After `install` (and any `reset`/`upgrade` followed by one), `git status` perpetually showed `M bin/halo` and `M litellm/config.yaml` — pure noise, no real content change. Root cause (empirically verified + §24 council): both are **git-tracked files that a runtime command regenerates**, and the committed copy had drifted from the generator. `bin/halo` is fully heredoc-generated by `installer/phases/11_halo_autoreason.sh` (`HALO_MODEL_DEFAULT="claude-opus-sub-xhigh"`), but the committed wrapper was stale at the old `local` default (never re-committed since the change); every `install` re-runs phase 11 and regenerates it. `litellm/config.yaml` is a derived artifact — `01_inference.sh` seeds it from `prompts/config.yaml` (only if absent), then `register_model_list` rewrites `.model_list` via `yq -i`, which re-serializes the doc and normalizes comment indentation; the committed file carried a hand-edited off-canonical (4-space) comment block, so every model render reindented it. (This exact class was fixed once for config.yaml — `bfc6944` — and regressed 5 days later via `fb41ce3`'s hand-edit; `reset`/`upgrade` don't write either file directly — the dirt comes from the install/model-render that follows.) **Fix:** re-sync committed `bin/halo` to the generator default (behavior-neutral — the installed wrapper already used it), and re-commit `litellm/config.yaml` in its yq-canonical form. To stop this recurring silently, two new drift-guard doctor checks make the invariants mechanical (dynamic count **80 → 82**, both print-only `_fix`, neither FIX_CAPABLE): **check 80 `halo_drift`** — asserts committed `bin/halo`'s `${HALO_MODEL:-…}` default matches `HALO_MODEL_DEFAULT` in the phase-11 generator (targeted-line assertion, no heredoc exec, since that heredoc is unquoted/interpolated — unlike check 62's literal `bin/audit.sh` body); **check 81 `config_canonical`** — asserts `litellm/config.yaml` is a yq fixed point (`yq '.'` re-emit == the file), which is exactly what `register_model_list` would write (verified against the live renderer output). Both guards mutation-tested (revert the fix → red; restore → green). §24 council of 4 (adversarial + architect + regression-historian + pragmatist) → consensus; it falsified two of the initial diagnosis's secondary claims (the fallbacks rationale essay lives only in the derived `config.yaml`, 6× vs 0× in the template, so gitignoring it — option C1 — would silently drop version-controlled knowledge + smuggle a fresh-install fallback-policy change; deferred as a recommended-against future hardening pending a deliberate template migration). Docs swept 80→82 across DOCTOR.md (+ new §80/§81), COMPONENTS/ONBOARDING/AGENT-ONBOARDING/TROUBLESHOOTING/ARCHITECTURE/INSTALL/TUTORIAL (+HTML regen).
...
- **Tests + doctor guards for the pipefail-EPIPE fixes (changes 2/3/4 shipped without them).** A §24 council flagged that the verify-then-stamp, install-logging, and doctor fix-capability changes shipped with no tests. Three hermetic, **mutation-tested** smokes close that: `installer/smoke/verify-then-stamp.sh` (pins that phases 39/40/41 err+exit-without-stamp on a failed post-condition, stamp on success, and — the highest-value line — a GENUINE no-sandbox/not-Ready fleet STILL stamps, the opt-in record that arms 04f; carries an always-on `VTS_MUTATE=1` self-control so it can never pass vacuously), `installer/smoke/install-log.sh` (pins the two facts found only by testing: a naive process-substitution tee loses the whole log, and a bare `wait` hangs the installer forever — plus exit-status/TTY/perms), and `installer/smoke/doctor-fix-capability.sh` (drives the REAL `doctor.sh` across capable/advisory/autoheal/NO_PROMPT via the `DOCTOR_CHECKS_DIR` seam). Two new doctor checks make the invariants mechanical (dynamic count **78 → 80**, both print-only `_fix`, neither FIX_CAPABLE): **check 78 `verify_then_stamp_guard`** — a source-shape drift guard (à la check 62) that all three phases keep the Ready-gate + post-condition gate + **an `exit` on the no-stamp-on-fail branch** (the §24 council caught the first cut was blind to exactly the incident's most direct reintroduction — deleting that `exit` left the check green; now caught) + the genuine-skip-still-stamps else-branch; **check 79 `fix_capable_integrity`** — every `doctor` run re-asserts the FIX_CAPABLE census (no orphan markers, AUTOHEAL ⊆ FIX_CAPABLE, every unmarked `_fix` is print-only via a source heuristic that also flags a write-redirect sentinel like `echo x > "$f"`), turning the earlier doc-template convention into an enforced control with zero false positives on today's 56 print-only bodies. Docs swept for the count bump across DOCTOR.md (+ new §78/§79 per-check sections), COMPONENTS/ONBOARDING/AGENT-ONBOARDING/TROUBLESHOOTING/ARCHITECTURE/INSTALL/TUTORIAL (+HTML regen). §24 council: SHIP-WITH-MUSTFIX → all must-fixes + should-fixes applied and re-verified.
- **Phase 40 (honcho) "cold code" de-risked and exercised.** The default-on flip made phase 40's enable path run on every `install all --include-optionals`, but that path had effectively never executed (the old gate flag was never set). Ran `install honcho_mcp` from main from its real cold state: it wired the fleet 10/10, the verify-then-stamp gate confirmed the wiring landed, `phase_40.done` stamped, and **full doctor went 78/78 with 0 skipped** (check 75 now a live pass, not a skip) — honcho joins docs + falkordb as a wired fleet-memory service. The one irreversible effect (retiring the raw `:8000` egress) was already applied from earlier work, so the exercised step was additive and reversible.
...
- **docs-RAG embedder family stamp + nomic task prefixes** — closes the canonical-768 council's strongest objection ("3c"). Doctor check 77 compared *dims*, so the dangerous case was invisible: re-point `docs` between the two **768** families (`embed-nomic` ↔ `embed-openai-small-768`), run `install 06` (which re-bakes `EMBED_MODEL`, satisfying the code-drift guard too) and **forget the re-index** — the store still held nomic vectors while queries embedded via OpenAI. Same dim ≠ same vector space: total, silent recall collapse with every check green. `ingest.py` now stamps the **models.yml registry key of the embedder that produced each vector** into that same point's payload (`embedder`), and check 77 counts any point not carrying the assigned embedder via the Qdrant `points/count` API. Provenance rides in the same write as the vector, so it cannot drift from what it describes, and it dies with the collection, so a dropped store can never present a stale stamp — chosen over a sidecar state file (drifts; no causal link to the store) and a reserved sentinel point (a real point in a similarity space — it would leak into `top_k` and needs an exclusion filter in every consumer). The stamp is excluded from the embedded text (`excluded_embed_metadata_keys`), so it never pollutes the space it audits. Semantics: entirely-unstamped = **skip-clean** (pre-dates the stamp), any wrong-family point = **FAIL**, partially-stamped = **FAIL** (mixed geometry). Defence in depth: `ingest.py` also **refuses** to append a different family onto a populated collection unless `AI_STACK_FORCE_RECREATE=1` — the doctor catches a forgotten re-index, the ingester prevents a mixed corpus. Separately, `nomic-embed-text`'s **asymmetric task prefixes** now ship: chunks embed as `search_document: `, queries as `search_query: `, applied **only** for the `embed-nomic` family (this version's `OpenAILikeEmbedding` has no `query_instruction`/`text_instruction` ctor arg, so the supported `_get_*_embedding` override hooks are used; empty prefixes make the OpenAI path an exact no-op, since prepending there would *hurt* recall). `EMBED_NAME` joins `EMBED_MODEL`/`EMBED_DIM` as a third baked literal so both generated files stay in lockstep. **`honcho` is deliberately NOT stamped** — it owns its embedding pipeline upstream and stamping it means patching code an upgrade overwrites; its dim guard + boot validator cover the dim case, and the same-dim family risk stays documented in the runbook rather than papered over with a fragile mechanism. Docs corrected while verifying: the re-index runbook told you to restart docs-mcp with a bare `start-docs_mcp.sh`, which **no-ops** when the pid is alive — `--recreate` is the arm that actually recycles it — and to `cp` the corpus back, which accumulates `*.1.md` duplicates via the no-clobber rename (now `mv`).

- **honcho memory is DEFAULT-ON — a deliberate POSTURE FLIP** (operator decision). Phase 40 (`honcho_mcp`) now wires by DEFAULT under `install all --include-optionals`, exactly like its sibling memory phases 39 (doc-RAG) and 41 (FalkorDB); it remains out of core `install all`. It was previously gated behind `HONCHO_MEMORY_OPT_IN=1` — **a flag NOTHING in the repo ever set** — so every blanket install silently DECLINED honcho while 39/41 wired themselves, and the gap was invisible (phase 40 exits 0; check 75 skip-cleans when unstamped), so the operator reasonably believed all three memory services were on when only two were. Enabling honcho changes the security posture two ways, and the phase now PRINTS both before acting rather than burying them: (a) it **retires the raw auth-off `honcho:8000` sandbox egress** — a net TIGHTENING, leaving the token-gated shim as the only in-sandbox path to Honcho; (b) it establishes **FULL-SHARED memory** — one workspace + a default `fleet` session, **no per-agent isolation**, so anything one peer remembers is recallable by every peer (keep per-agent secrets out). Decline with `HONCHO_MEMORY_OPT_IN=0 mayssam-ai-stack.sh install all --include-optionals`; the decline path deliberately stamps NOTHING (check 75 keys its skip-clean on `phase_40*.done`, so a stamped decline would red-bar a stack that opted out). Docs re-synced to the code: `services.yml` help (which had asserted the EXACT OPPOSITE — that an explicit flag meant `--include-optionals` "cannot flip it silently"), EXPLORE, TUTORIAL (+regen), DOCTOR check 75, TROUBLESHOOTING.
- **install runs are now logged.** `install` tees its full stdout+stderr to `installer/state/install-<UTC-ts>.log` with `installer/state/install-latest.log` symlinked to the most recent run, and `--include-optionals` records the phases it attempted to `installer/state/install-optionals-last.txt` — so a silent decline or a mid-run abort is reconstructible after the fact instead of scrolling off the terminal. (`/installer/state/` was already gitignored.)
...
- reasoning-effort drop closed (silent degradation, born at install time) — on any box whose `litellm/config.yaml` was last written by `install inference` / `install all`, the three effort-declaring OpenAI routes (`openai-gpt`, `openai-gpt-sub`, `openai-gpt-pro-sub`) are served with **no** `reasoning_effort` at all while `installer/models.yml` declares `effort: xhigh`: they answer normally, just at the provider's default reasoning, with no error, no log line, and no failing check. Confirmed on this machine against the RUNNING proxy — `GET /model/info` reported `<absent>` for all three while the meridian ladder correctly served `max`/`high`. The **tracked** config.yaml was never wrong: `install inference` REWRITES those rows and strips effort, and `model sync` is never auto-run by install, so a fresh install produces the degraded config and nothing restores it until an operator manually syncs. Root cause: `installer/phases/01_inference.sh` hand-mirrors `lib/models.sh::register_model_list`'s argument extraction (there are THREE copies of that extraction — `register_model_list`, `models.sh::_dry_run`, and phase 01) and phase 01's `case` arm read `effort` for `meridian` ONLY, so the openai/codex-bridge routes registered with an empty effort — and because `lib/lmstudio.sh::lms_register_model` **reassigns `litellm_params` to a fresh map**, the omitted arg wasn't skipped, it was DELETED. The 7 meridian models were unaffected (their arm existed), which is exactly why nothing caught it: check 41 guards `extra_body.effort` but is RUNTIME-scoped (it skips every non-meridian model — it is not gated on Meridian being enabled, and its clause runs regardless of daemon state), so the openai half had no guard at all; and `installer/smoke/models-gpt.sh` asserts the contract but calls `lms_register_model` DIRECTLY, testing the writer, so it structurally cannot catch a broken caller. Fix: phase 01's arm now reads `meridian|openai|codex-bridge`, mirroring `register_model_list` exactly (the comment records that this block has drifted twice, and that a follow-up should delete the hand-mirror outright), plus a runtime-agnostic **effort round-trip guard** in doctor check 40 asserting BOTH layers, which fail independently: the FILE (what the next boot serves → remedy `model sync`, explicitly NOT `install inference`, which self-gates on its stamp and exits before re-registering) and the RUNNING PROXY via `/model/info` (what it serves now → remedy `start-litellm.sh --recreate`), since LiteLLM reads config at boot and phase 01 deliberately never restarts a healthy container — so "file repaired, proxy stale" would otherwise green. `/model/info` is static route metadata: no inference, no cold-start. An `effort` on a runtime that renders none is an advisory. Verified: the guard flags exactly the 3 broken routes and 0 on a correct config, under the doctor runner's strict mode.
- **`set -o pipefail` + EPIPE made sandbox detection lie ~80–90% of the time — the root cause of the phase-39/41 unwired-fleet incident.** `openshell sandbox list | grep -q NAME` returns FALSE **even when grep MATCHED**: `grep -q` exits the instant it matches (usually a middle row), the Rust `openshell` writer takes EPIPE, and because it sets `SIGPIPE=SIG_IGN` it PANICS and exits **101** instead of dying quietly — `pipefail` then promotes that 101 to the pipeline's status, so the `if` is false (`PIPESTATUS=[101 0]`), and the `2>/dev/null` on the probe hid the panic entirely. Phases 39/41 therefore skipped fleet MCP wiring, **stamped `.done` anyway**, and doctor red-barred checks 74/76 with no visible cause. Fixed with `sandbox_present()` in `installer/lib/common.sh` — an awk form that DRAINS stdin, so the writer never sees EPIPE — replacing the racy probe at 7 sites across phases 04/30/39/40/41. Covered by `installer/smoke/sandbox-present.sh` (hermetic + mutation-tested). NB the race only reproduces under `/bin/bash`, never under zsh.
- **VERIFY-THEN-STAMP in phases 39/40/41.** A phase may no longer stamp `.done` on the strength of its own optimism: the stamp is now gated on the doctor's own assertion that the wiring really landed, so a silently-skipped phase can't mark itself complete. The GENUINE no-sandbox branch still stamps and exits 0 by design — that stamp is the OPT-IN RECORD that arms `04f` to wire a fleet built LATER — and an optional extra's failure still never aborts a core install.
- **Doctor's auto-fix prompt now gates on CAPABILITY, not on a fix merely existing.** Doctor offered *"Auto-fix available. Apply? [Y/n]"* for any check defining a `<name>_fix` (`declare -F`) — but 54 of the 75 fix bodies only PRINT guidance, so the operator answered `y`, nothing ran, and doctor reported *"fix ran but the check still fails"* (the 74/76 incident). A fix must now declare `FIX_CAPABLE[<name>]=1` to be offered; census of the 78 checks: **21 capable** (19 prompt-and-really-fix + the 2 AUTOHEAL — `05a` litellm_keystore, `73` hermes_workspace_pair — which apply automatically and are unchanged), **54 advisory**, 3 with no fix at all. An UNMARKED check is treated as advisory, so a new check can never over-promise a fix it cannot perform. Behavior change worth knowing: advisory checks now print their guidance under `NO_PROMPT` / non-interactive too, under an honest `Manual step required:` heading — previously they printed nothing there while still claiming an auto-fix was available. `NO_PROMPT=1` remains non-mutating.
...
`mayssam-ai-stack.sh install falkordb_mcp`.
...
- **Docs swept** — `services.yml` (`falkordb_mcp`), `doc/EXPLORE.html` (memory tier, 62 → 63 cards;
  ...
  help line in `mayssam-ai-stack.sh`.
...
`HONCHO_MEMORY_OPT_IN=1 mayssam-ai-stack.sh install honcho_mcp`.
...
- **Docs swept** — `services.yml` (`honcho_mcp`), `doc/EXPLORE.html` (memory tier, 61 → 62 cards),
  ...
  and the opt-in-extras help line in `mayssam-ai-stack.sh`.
...
**This slice — claude-cli memory (opt-in Phase 39, `mayssam-ai-stack.sh install fleet_memory`):**
...
User report: "the script failed when I called the doctor command multiple times." `mayssam-ai-stack.sh doctor`
...
sibling commands, e.g. `mayssam-ai-stack.sh url <unknown>` no longer stacks a `✗ ERR line …` on top of `bin/url`'s
own error.
...
- **bash-5 gate (complete).** Every sub-script dispatch in `mayssam-ai-stack.sh` AND in
...
`mayssam-ai-stack.sh tutorial-serve` (and the sibling foreground commands) printed a
...
- **Rollback**: set `HERMES_SLACK_ROLE_ROUTER=false` in `.env`, re-run
  `mayssam-ai-stack.sh install 38`, and Phase 38 will use upstream Hermes native Slack
...
- **Install-lock stale-reclaim** — the watchdog's `[[ -d $INSTALL_LOCK ]] && exit` deferred **forever**
  ...
  stall. Now defers only for a LIVE install (pid alive AND a `mayssam-ai-stack` process — closes the
  PID-reuse "defer-for-hours" hole) with a young-lock grace for `lock_acquire`'s mkdir→write-pid race.
- **Owner admin commands** — `mayssam-ai-stack.sh hermes config {snapshot|restore|show}` and
...
- **`mayssam-ai-stack.sh ingress list`** — every configured hostname, all three URL forms, live
...
- **`mayssam-ai-stack.sh ingress add <name> <port>` / `remove <name>`** — register any `localhost:port`
...
- **`mayssam-ai-stack.sh url`** is now a top-level command (was `bin/url` only); `bin/url` honors
...
- **NEW Phase 38 `38_hermes_slack.sh`** (opt-in — NOT in `install all`) — writes
  ...
  allowlist in sync). Install: `mayssam-ai-stack.sh install 38` (alias `install slack`).
...
- **Understand-Anything — cross-runtime codebase knowledge graphs (opt-in Phase 30)** (`feat/understand-anything-phase30`): integrates the [Understand-Anything](https://github.com/Lum1104/Understand-Anything) Claude Code plugin so a codebase knowledge graph is usable from **every runtime, not just Claude Code**. Spec: `docs/superpowers/specs/2026-06-21-understand-anything-phase29-design.md` (2 §24 councils: design + implementation). **Architecture = generate centrally, consume everywhere.**
  ...
  - **(d) Lifecycle + docs** — `mayssam-ai-stack.sh install understand` (opt-in; needs Node ≥22 + pnpm; builds plugin core + shim, mints `UNDERSTAND_MCP_TOKEN`, registers stdio MCP, starts the daemon, wires the fleet; idempotent + reversible), `start/stop understand` (node-bg daemon `bin/start-understand.sh`), `understand-dashboard` (browser graph), `test understand` (real `graph_search` from inside a Hermes profile — `smoke/30.sh`). Doctor check **52** (3-state, true headless E2E query, WARN-equivalent when no graph yet). Runbook `doc/UNDERSTAND.md`. Boundary: understand = orientation/architecture map; Sourcegraph/lumen = raw source retrieval.
  - Files: `installer/phases/30_understand.sh`, `installer/lib/mcp.sh` (+fn), `bin/start-understand.sh`, `installer/lib/understand-dashboard.sh`, `installer/doctor/checks/52_understand.sh`, `installer/smoke/30.sh`, `services.yml` (`understand`: node-bg, host, :7081), `mayssam-ai-stack.sh` (dispatch), `understand-mcp/`. Counts: **43→44 services, 52→53 doctor checks, 8→9 opt-in extras**. **Pending live verify from main:** generate + commit the ai-stack sample graph, `doctor` 53/53, `test understand` fleet E2E.
...
- **`install` / `doctor` no longer block on a global-`docker context` prompt** (`worktree-fix-docker-context-prompt`): `engine_pin` (called from Phase 00 preflight, `deps.sh`, and doctor checks 01/47/48 — i.e. *every* `install`/`doctor`) prompted interactively *"Also point your global `docker context` at OrbStack? [y/N]"* and **waited for a keypress mid-run**. That choice is now a **persisted, non-interactive preference**: **`AI_STACK_DOCKER_CONTEXT`** in `.env` (`switch` = default: silently point the global context at `ai-stack-<engine>`, recording the prior context once in `AI_STACK_DOCKER_CONTEXT_PRIOR` for a clean undo; `keep` = never touch it; an unrecognized value fails safe to `keep`). The process env var overrides `.env` for one-off runs. Set it in **`setup`** (asks once) or via the new **`mayssam-ai-stack.sh docker-engine context [status|switch|keep]`** subcommand (`keep` also restores the recorded prior context). `engine_pin` never calls `read` again — a structural smoke-test guard (`installer/smoke/engine.sh`) enforces it, alongside stub-`docker` tests for keep/switch/idempotent/restore. `docker-engine status` now also shows the policy. Spec: `doc/specs/2026-06-21-docker-context-policy.md`; docs swept (README, PREREQUISITES, both engine-selection specs marked superseded). §24 council reviewed → SHIP.
...
- Doc-drift sync (24 → 25): hand-edited the canonical sources — `agent-profiles/SOUL-SUPERSET.md`, `agent-profiles/hermes/profiles/manager/SOUL.md`, `doc/specs/2026-06-11-manager-second-brain.md` (×2) — plus 2 project-memory files. Derived `pi/` + `claude-code/` copies and the installed `~/.claude/` files (incl. `~/.claude/fleet/manager.md`, still reading "24 rules") were deliberately NOT hand-edited, per the source→derived sync model guarded by `check_fleet_parity.sh`. **User step:** run `bash mayssam-ai-stack.sh install 04h` to regenerate the 3-fleet derived copies and refresh the live `~/.claude/` environment.
...
tracked + committed. Live re-sync (`mayssam-ai-stack.sh install agent_fleet` → `~/.claude`, pi-v1,
...
opus/sonnet `-high` split + "ACE/RLM stay local". Verified against `mayssam-ai-stack.sh model list`
...
- **EXPLORE.html:** docs_mcp + llm_guard cards now show `mayssam-ai-stack.sh start <svc>` as the run
...
**Result: `mayssam-ai-stack.sh doctor` = 45/45, 0 failed, 0 skipped** (verified live).
...
**Run/stop contract — `mayssam-ai-stack.sh start <svc>` is the single way to run anything:**
...
`lms server start` as the run path — they point at `mayssam-ai-stack.sh start lmstudio` (LMS_AUTOSTART
...
Install: `bash mayssam-ai-stack.sh install 26` then `bin/mempalace wake-up`.
...
3. **claw3d UI is now name-aliasable** as `http://claw3d:4310` (added to
   `installer/lib/aliases.tsv`, `127.0.10.17`; activates after `sudo mayssam-ai-stack.sh
...
**Named phase selectors.** `mayssam-ai-stack.sh install <phase>` and `test <phase>` now accept a
...
unique-fuzzy; new `mayssam-ai-stack.sh phases` (also `steps`/`list`) prints the `id → name` table.
...
- **`services.yml`** `hermes_telegram` (`type: sandbox-daemon` → `status` shows `n/a`
  ...
  the `sandbox-daemon` type. mayssam-ai-stack.sh phase array → `…19 20`.
...
- **NEW doctor check 32** (`32_claw3d.sh`); `services.yml` entries (`claw3d`,
  `claw3d_bridge`); mayssam-ai-stack.sh phase array → `…18 19`. **Phases 25 → 26, checks 31 → 32.**
...
- **NEW doctor check 31** (`31_rlm.sh`): venv imports `rlm` + `bin/rlm` + `rlm/.env`
  ...
  **Doctor count 30 → 31.**
- **mayssam-ai-stack.sh** phase array + usage now include `18`; **services.yml** has an `rlm`
...
5. **Non-interactive reset** (`prompt.sh`, `mayssam-ai-stack.sh`, `reset.sh`). `confirm()`
...
- All docs → `doc/` **except `README.md` + `CHANGELOG.md`** (stay at root;
  CHANGELOG stays where `mayssam-ai-stack.sh` appends). 15 files moved; all 73 inter-doc
...
- Modified: `mayssam-ai-stack.sh`, `installer/lib/{prompt,reset,network,bootstrap}.sh`,
...
- `installer/lib/docker.sh::container_exists` + `container_managed` are still single-container-only. Other call sites (`mayssam-ai-stack.sh adopt`, `stack stop`, doctor checks) may have the same compose blindspot. Worth a follow-up sweep.

---

## 2026-05-29 — Phase 17 ACE + Hermes/Pi via PyPI inside sandbox venv + mayssam-ai-stack.sh E2E verified

User feedback: "wtf does it mean hermes doesn't work. that's the primary thing i use. please fix all these things. paperclip and deerflow both should be enabled. mayssam-ai-stack.sh should work. hard reset and test it once you finished with fixes. use 3 other agents to work together as a team to review each other work every time. don't forget about mappings like litellm:4000 → litellm/ finally i want to use this as well add this to my stack: https://github.com/ace-agent/ace"
...
4. **mayssam-ai-stack.sh** — `17` added to default phase array (line 325) and usage help (line 178). Auto-discovery handles run_phase + doctor.
...
$ bash mayssam-ai-stack.sh doctor | tail -3
...
- Modified: `installer/phases/04f_hermes_fleet.sh` (PyPI + per-file upload + bootstrap.sh body), `installer/phases/15_pi.sh` (load_env→get_env + ANSI strip), `mayssam-ai-stack.sh` (phase array + usage), `services.yml` (11 host_port fixes + ace entry)
...
3. **`mayssam-ai-stack.sh start|stop` subcommands** — `cmd_start <svc>` execs `bin/start-<svc>.sh`. `cmd_stop <svc>` prefers `bin/stop-<svc>.sh`, falls back to `docker stop <svc>`. Both surface a list of available services on misuse.
...
```
$ bash mayssam-ai-stack.sh stop deerflow
[deer-flow containers stopped]
$ bash mayssam-ai-stack.sh deerflow start    # reverse-form
[no WARN lines]
...
$ bash mayssam-ai-stack.sh doctor deerflow_config | tail -1
...
- `mayssam-ai-stack.sh` — new `cmd_start` / `cmd_stop` (+ `_list_startable_services`), wired `start|enable|stop|disable` into the dispatcher, reverse-form fallback in `*)`. Usage text refreshed.
...
- Fix function points at re-running `bash mayssam-ai-stack.sh install 10`.

### Verification (live)

```
$ bash ~/ai-stack/mayssam-ai-stack.sh doctor | tail -3
  DeerFlow config.yaml has model entries + compose passes LITELLM_MASTER_KEY (Phase 10) ✓
...
from `mayssam-ai-stack.sh complete` to a working multi-service workflow.
...
- **Daily cheatsheet**: 11 commands verified against `mayssam-ai-stack.sh` +
...
LOCAL-type gateway (e.g. `openshell` from the official mayssam-ai-stack.sh) instead
...
1. **Phase 14 — Unsloth Studio** (`installer/phases/14_unsloth_studio.sh`).
   ...
   `bin/start-unsloth.sh`. Standalone: `bash mayssam-ai-stack.sh install 14`.
...
`brew install openshell` works via the official `mayssam-ai-stack.sh` now; the
...
when the gateway isn't up; the user can re-run `bash mayssam-ai-stack.sh install
...
- **Phase 00·V — runtime verification pre-flight**
  ...
  Failure prints the exact fix command (`sudo bash mayssam-ai-stack.sh prepare-sudo`
  for the common case) and exits 1 *before* a single Phase 01 container starts.

### New mayssam-ai-stack.sh subcommand

- `bash mayssam-ai-stack.sh verify` — runs Phase 00·V standalone (and clears its
...
- **Modified**: `installer/lib/aliases.tsv` (host_port=container_port for
  HTTP), every `bin/start-*.sh` (no more `:80:` literal), `mayssam-ai-stack.sh`
...
For installs that ran the 2026-05-27 entry: `bash mayssam-ai-stack.sh verify` will
fail loudly on every alias whose host_port was `:80`. Fix:

```bash
bash mayssam-ai-stack.sh reset --confirm hard    # tears down managed containers
sudo bash mayssam-ai-stack.sh prepare-sudo       # re-runs lo0_ensure_aliases + plist
bash mayssam-ai-stack.sh install all             # phase 00·V will probe; phase 01 onwards re-publishes on native ports
```
...
1. `bash mayssam-ai-stack.sh install 00n` — creates the `ai-stack` Docker network,
...
3. `bash mayssam-ai-stack.sh adopt <svc>` per foreign container — re-creates with
...
exact `bash mayssam-ai-stack.sh adopt <svc>` commands to run.
...
**Outcome**: `bash mayssam-ai-stack.sh install all` runs end-to-end and stamps 17/18
...
1. **Foreign containers (litellm, phoenix, falkordb, qdrant)** — started by
   ...
   `bash mayssam-ai-stack.sh adopt <svc>` per service to take ownership (each adoption
...
2. **PHOENIX_API_KEY empty** — Phoenix is running with auth ON. The OTLP
   ...
   as `PHOENIX_API_KEY=…` → run `mayssam-ai-stack.sh apply-restarts` to recreate
...
recreate is required. Run `bash mayssam-ai-stack.sh apply-restarts` to drain.)
...
- `mayssam-ai-stack.sh test 01` — 23/23 models served, local chat completion + trace
...
- `mayssam-ai-stack.sh test 02` — Qdrant create+list+delete; FalkorDB PING +
  GRAPH.QUERY/DELETE.
- `mayssam-ai-stack.sh test 03` — Honcho /health 200.
- `mayssam-ai-stack.sh test 05` — Open WebUI 200.
...
7. **Adoption flow** for containers started outside the installer (i.e. the four already-up containers): `mayssam-ai-stack.sh adopt <svc>` does `docker cp` → diff vs declared → require `yes` confirmation → backup stateful data → `rm -f` → recreate via managed start script. Without adoption, doctor reports "foreign container — run `mayssam-ai-stack.sh adopt <svc>` to take ownership." (Reviewer Adversarial #1+#2.)
...
9. **Container labels** for every managed container: `ai-stack.managed=true`, `ai-stack.phase=NN`, `ai-stack.partial=true` (removed after smoke test passes). `mayssam-ai-stack.sh gc` lists & cleans `partial=true` orphans. (Reviewer Adversarial #11.)
...
11. **Per-model smoke for LiteLLM**: `mayssam-ai-stack.sh test 01` issues `messages=[{role:user,content:"ping"}], max_tokens:1` for each declared `model_name`. Records PASS/FAIL/SKIP-NO-KEY. Failures don't fail the phase (provider deprecations are common) but doctor surfaces them. (Reviewer Adversarial #10.)
12. **Downstream-restart queue**: when a phase mutates `.env` in a way that requires restarting an upstream-installed service (e.g., phase 03 sets `HONCHO_API_KEY`, but litellm was started in phase 01 with empty), the installer writes the affected service name to `installer/state/restarts-needed.txt`. End-of-install prints the list with `mayssam-ai-stack.sh apply-restarts` to drain. Never auto-restarts in conservative mode. (Reviewer Adversarial #12.)
...
14. **CHANGELOG races avoided**: per-run files in `CHANGELOG.d/<run-id>.md` rather than appending to `CHANGELOG.md`. `mayssam-ai-stack.sh history` compiles. (Reviewer Adversarial.)
