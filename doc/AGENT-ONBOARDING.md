...
|---|---|---|
| Services | **49** in `services.yml` (~25 networked; ~24 are `network:none` patterns/features/keys) | `yq '.services\|keys\|length' services.yml` |
| Doctor checks | **66** (dynamic — a new check file auto-bumps it) | `bash vz-ai-stack.sh doctor` |
| Phase files | **44** (00–36 + sub-phases; ~29 core / ~15 opt-in) | `bash vz-ai-stack.sh phases` |
...
├── services.yml            # 49 services + profiles (source of truth)
...
Hand this to a fresh agent (a new Claude Code session in `~/ai-stack`, or any capable coding
agent). It self-onboards using this file.

```text
You are the incoming OWNER and MAINTAINER of `ai-stack` — a local-first, self-hosted AI platform
that turns one Apple-Silicon Mac into a private AI cloud (~49 services behind a single LiteLLM
