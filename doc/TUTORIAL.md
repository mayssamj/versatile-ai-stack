...
This is where you go from a clean Apple-Silicon Mac to a fully healthy, self-hosted AI platform — all 46 services running behind one local endpoint, with zero bytes leaving the building. By the end of Act I you'll understand the mental model, have the host prepared, the stack installed, and a green doctor proving it.
...
3. The ~46 services sort into layers:
...
```bash
# HEALTH — run the full diagnostic sweep (59 checks, each self-diagnosing).
