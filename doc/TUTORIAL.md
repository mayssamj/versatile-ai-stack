...
This is where you go from a clean Apple-Silicon Mac to a fully healthy, self-hosted AI platform — all 54 services running behind one local endpoint, with zero bytes leaving the building. By the end of Act I you'll understand the mental model, have the host prepared, the stack installed, and a green doctor proving it.
...
3. The ~54 services sort into layers:
...
- `host.docker.internal:7082` — **honcho-mcp** (memory shim, token-gated), with a `pi` peer namespace. Raw Honcho `:8000` is **denied**.
- `host.docker.internal:7083` — **falkordb-mcp** (graph-memory shim, token-gated). Raw FalkorDB `:6379` is **denied**.
- `host.docker.internal:8765` — docs-mcp (unauthenticated, read-only).
- pypi / npm / github — extension installs and reference-repo clones only.

Pi **cannot see** phoenix, qdrant, openwebui, the workspace, unsloth, paperclip, autofyn, or any other stack service — nor **raw** Honcho (`:8000`) or **raw** FalkorDB (`:6379`). It reaches both memory planes *only* through the token-gated shims above. It's a coding agent in a box that can think (via LiteLLM) and remember (via the honcho-mcp / falkordb-mcp shims) but can't reach your other services or exfiltrate a key. Emergency stop: `bin/pi-kill`.

**Its memory tools (slice 2b).** Pi ships no MCP client, so `pi/pi-memory-tools.ts` bridges MCP-over-HTTP into native Pi tools: `search_documents` (docs-mcp), `honcho_remember` / `honcho_recall` / `honcho_ask` / `honcho_search` (honcho-mcp), and `remember_fact` / `recall_related` / `graph_query` (falkordb-mcp). `graph_write` is **withheld** — it runs arbitrary destructive Cypher against the one shared, un-backed-up fleet graph, so it registers only under the explicit `PI_MEMORY_ALLOW_GRAPH_WRITE=1` opt-in. `bin/pi` injects `HONCHO_MCP_TOKEN` / `FALKORDB_MCP_TOKEN`; a token-less Pi simply degrades to fewer tools rather than failing.
...
```bash
# HEALTH — run the full diagnostic sweep (85 checks, each self-diagnosing).
...
mayssam-ai-stack.sh install omp            # 42 — oh-my-pi terminal coding agent (Pi hard fork) on your stack models
```

**Expected.** Each phase is idempotent and exits 0 cleanly even when a prerequisite is missing (it warns and *does not stamp*, so a later re-run completes). These extras' doctor checks are **34–38** (plus **49** sourcegraph, **50** aionui, **51** openwork, **52** understand, and the agent-swarm-sims **57** metagpt, **58** agentscope, **59** oasis, **60** chatdev, **61** aitown, **66** concordia, plus **84** omp) — each runs only once the corresponding extra is installed. (MemPalace is no longer in this list — it's part of `install all`, with its own check 44.) The six agent-swarm simulators (metagpt/agentscope/oasis/chatdev/aitown/concordia) get a full hands-on in **L20½**.
