...
| `litellm`                     | **Everything chat-related stops.** Open WebUI shows API errors; Honcho deriver background work pauses; Hermes profiles in sandbox can't think; docs_ingestor / docs_mcp can't embed; audit.sh fails. | `bash bin/start-litellm.sh` (or `vz-ai-stack.sh apply-restarts`). Check `docker logs litellm` for ImportError on guardrails.py. |
