...
| `pi` (sandboxed)      | openshell sandbox `pi-v1` | n/a (no direct ollama)        | `http://host.docker.internal:4000/v1` / `Bearer PI_LITELLM_KEY` (virtual key; models=[local, local-heavy, local-lfm2]) | n/a                                          | `http://host.docker.internal:8000` / `pi` peer namespace (soft isolation; Honcho v3 has no per-key peer enforcement) |                            | `http://host.docker.internal:8765` / n/a | n/a (policy denies)                              |
...
|-------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------|
| `ollama`                      | `model=local` and `model=local-heavy` requests fail with connection refused on `ollama:11434`. Cloud-model requests unaffected.                  | `brew services restart ollama` then `curl http://ollama:11434/api/tags`. LiteLLM has no `local` fallback by design (CHANGELOG cites this). |
