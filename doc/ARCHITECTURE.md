...
per-agent assignment table; `installer/lib/models.sh` is the `mayssam-ai-stack.sh
...
(`mayssam-ai-stack.sh model superset` prints it — the union of the legacy
...
```
~/ai-stack/
├── mayssam-ai-stack.sh                       — entry point, dispatcher, NO logic
...
│   │   ├── deps.sh                  — `mayssam-ai-stack.sh deps`: host-dependency verify/install/start/re-verify
│   │   ├── setup.sh                 — `mayssam-ai-stack.sh setup`: interactive, skippable .env / API-key bootstrap
...
│   │   ├── verify.sh                — runtime-probe helpers (used by Phase 00·V + mayssam-ai-stack.sh verify)
...
│   │   ├── status.sh                — `mayssam-ai-stack.sh status` (run via bash)
│   │   ├── adopt.sh                 — `mayssam-ai-stack.sh adopt <svc>` (interactive)
│   │   ├── gc.sh                    — `mayssam-ai-stack.sh gc` (partial orphan cleanup)
│   │   ├── history.sh               — `mayssam-ai-stack.sh history` (assemble CHANGELOG.d)
│   │   └── reset.sh                 — `mayssam-ai-stack.sh reset --confirm soft|hard|nuke`
│   │
...
│   │       └── 23_… 84_omp.sh  — full list in doc/DOCTOR.md (85 checks total)
│   │
...
│   ├── stack                        — wrapper: `exec bash mayssam-ai-stack.sh "$@"`
...
Edit `services.yml`, run `bash mayssam-ai-stack.sh status` — drift is visible
immediately.
...
- Self-contained — can run standalone via `bash mayssam-ai-stack.sh install <phase>`.
...
- `env.sh`: get_env / set_env / require_env / env_hash / load_env_strict / fix_crlf;
  ...
  and `mayssam-ai-stack.sh setup`.
- `deps.sh`: `mayssam-ai-stack.sh deps` — bootstraps host dependencies (verify → install → start → re-verify).
- `setup.sh`: `mayssam-ai-stack.sh setup` — interactive, skippable `.env` / API-key bootstrap; always
...
and then runs `mayssam-ai-stack.sh install all`. Without the precheck, the stamp would
...
command — usually `sudo bash mayssam-ai-stack.sh prepare-sudo` — and exits 1
*before* a single Phase 01 container starts.

### `mayssam-ai-stack.sh verify` subcommand

`bash mayssam-ai-stack.sh verify` runs Phase 00·V standalone (and clears its own
...
`sudo bash mayssam-ai-stack.sh prepare-sudo` is the one-shot path that handles
every operation that requires `sudo`. After it succeeds, the rest of
`mayssam-ai-stack.sh install all` runs without prompting. It:
...
- `ai-stack.managed=true` — this installer owns it. (Foreign containers
  ...
  `mayssam-ai-stack.sh adopt` to take ownership.)
- `ai-stack.phase=<NN>` — which phase installed it.
- `ai-stack.partial=true` — set at create, removed by `mark_ready` after
  smoke test passes. `mayssam-ai-stack.sh gc` cleans `partial=true` orphans.
...
`bash mayssam-ai-stack.sh adopt <svc>` is the path for a container that was started
...
```
⚠ Queued restarts pending (run 'mayssam-ai-stack.sh apply-restarts'):
    litellm
```

User runs `bash mayssam-ai-stack.sh apply-restarts`, which executes
...
- **No automatic config migration.** `services.yml` has a `version: 1` field
  ...
  `installer/migrations/v1-to-v2.sh` and detect at mayssam-ai-stack.sh startup.
