...
**✅ FIXED in-code 2026-06-06 (`f7dc329`) — the install path now SELF-HEALS the relay timeout.** Root cause of the timeout: the sandbox's short-lived gateway token EXPIRED. The sandbox still reports `Phase: Ready` (control-plane) and `policy set` still works, but the gateway→sandbox **exec relay is dead**, so every `exec` times out at ~10s — and it can be dead at **LOW CPU (~0.2%)**, so the CPU-based storm detector misses it. The token **cannot self-refresh**; only RECREATING mints a fresh one. This used to fail `install all` at 04f (and was MISreported as a PyPI/pip 403). Now `installer/lib/openshell.sh` has `openshell_relay_ok` (bounded, hang-safe trivial-exec probe) + `openshell_token_storm` (log-signature detector); `openshell_sandbox_ensure` probes the relay on `Ready` and **recreates on a confirmed token-storm** (retries 40s first if the signature is absent, so a slow-but-healthy sandbox isn't needlessly nuked). Prechecks in phases **04/04f/15** probe the relay so a stamped-but-dead sandbox re-runs its body → `ensure` heals it. **So the fix is simply: `bash vz-ai-stack.sh install 04 04f` (hermes) / `install 15` (pi) — it now detects + recreates the dead sandbox automatically.** (Recreation still DISCARDS in-sandbox `/sandbox`+`/tmp` scratch — no snapshot; persist to host/Honcho.) Verified live on both real storming sandboxes. STILL TODO: proactive token refresh + self-heal at interactive entry points (`bin/pi`, `fleet`/`hermes` launch) — today only `install` + the opt-in watchdog heal. See [[project_cpu_gotchas]].
