...
- `doctor [<filter>]` — 59 checks, per-check auto-fix
...
2. `stack doctor` — 59 checks, each with auto-fix offer.
...
| `services.yml` | Single source of truth: 46 services with type/enabled/path/port/project/process_pattern/etc. (schema `version: 2`) |
