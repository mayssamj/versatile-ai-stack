...
| Last verified doctor pass | 2026-05-31: a fresh `install all` reaches **37/37** (Phase 18 RLM + Phase 19 claw3d + Phase 20 Telegram; checks 31/32/33 verified individually). ⚠️ A long-idle live stack can show 2 OpenShell-exec checks (25 pi-v1, 30 hermes) failing — that's the §2.1 relay idle-timeout, NOT a regression; a reset clears it. |
| Last verified cold install | 2026-05-31 — `reset --confirm hard --yes` → `install all` → `doctor` 37/37 green, end-to-end (Phases 19 claw3d + 20 Telegram verified; check 33 skips→passes when no `HERMES_TELEGRAM_BOT_TOKEN`. See CHANGELOG 2026-05-31) |
...
- `doctor [<filter>]` — 37 checks, per-check auto-fix
...
# 4. Verify (37/37 expected)
bash ~/ai-stack/install.sh doctor
```

This canonical flow is VERIFIED end-to-end (37/37 doctor, 2026-05-31). OpenShell sandbox-create hangs are now auto-recovered in-code (§2.2), so step 3 should no longer stall there.
...
2. `stack doctor` — 37 checks, each with auto-fix offer.
...
| `services.yml` | Single source of truth: 38 services with type/enabled/path/port/project/process_pattern/etc. |
...
| `installer/doctor/checks/NN_*.sh` | One per failure mode (37 checks). Each defines `CHECKS+=(name)` + `<name>_diagnose` + `<name>_fix` |
