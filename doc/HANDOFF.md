...
- `doctor [<filter>]` — 50 checks, per-check auto-fix
...
# 4. Verify (50/50 expected — #05a litellm_keystore AUTOHEALS the LiteLLM key-store/Postgres (runs before the per-phase key checks); MemPalace check #44 runs once Phase 26 is installed (green-skips only on a pre-change/partial install); #45 tutorial is always-on; #46 = agent_fleet_parity; #47/#48 = docker-engine consistency/selection; #49 sourcegraph_mcp skip-cleans when SG not installed)
...
2. `stack doctor` — 50 checks, each with auto-fix offer.
