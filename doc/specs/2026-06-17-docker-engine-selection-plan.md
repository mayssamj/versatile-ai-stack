...
> **⚠️ AS-BUILT RECONCILIATION (read this — it supersedes the 47/48 / 46→48 numbering throughout this plan):** This plan was authored on a transient branch where `46_agent_fleet_parity.sh` was present (count 46). It was then implemented on `feat/docker-engine-selection`, whose real baseline is **45** (`46_agent_fleet_parity.sh` lives on a *separate, unmerged* branch and is NOT here). Task 0's gate caught this. **As built:** the two new checks are **`46_docker_engine_consistency.sh`** + **`47_docker_engine_selection.sh`**, final count **47**, every doc count swept **45→47**. Wherever this plan (and its Debate/Review logs below) says "47/48", "count 46→48", or "46_agent_fleet_parity intact", read it as **46/47, count 45→47**. The spec, the code, `DOCTOR.md`, and the commit messages all reflect the as-built 46/47. If the fleet-parity branch later merges to main, whichever of {fleet-parity, this feature} merges second renames its `46_` to the next free ordinal (standard parallel-branch reconciliation).
