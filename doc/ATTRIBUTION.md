...
| **Concordia** | https://github.com/google-deepmind/concordia | Apache-2.0 | Google DeepMind. Generative Agent-Based Modeling (GABM) — LLM agents in a shared world mediated by a Game Master that validates/resolves actions against world-state. Host uv venv (py3.12, the first 3.12 sim); `gdm-concordia` 2.4.0 + `sentence-transformers` embedder (all-MiniLM-L6-v2, 384-dim). Routes through LiteLLM (scoped key) → Phoenix; default model `claude-sonnet-sub-high` (Concordia's concurrent per-step calls time out on a single local model). Opt-in (Phase 37). |
