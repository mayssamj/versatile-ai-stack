...
## Opt-in experimental extras (Phases 21–25 · 27–38)
| Component | Upstream | License | Notes |
|---|---|---|---|
...
| **Concordia** | https://github.com/google-deepmind/concordia | Apache-2.0 | Google DeepMind. Generative Agent-Based Modeling (GABM) — LLM agents in a shared world mediated by a Game Master that validates/resolves actions against world-state. Host uv venv (py3.12, the first 3.12 sim); `gdm-concordia` 2.4.0 + `sentence-transformers` embedder (all-MiniLM-L6-v2, 384-dim). Routes through LiteLLM (scoped key) → Phoenix; default model `claude-opus-sub-xhigh` (opt-in sim default; the install gate uses the faster `claude-sonnet-sub-high` — Concordia's ~26 concurrent calls/step time out on a single local model and would blow the gate at opus xhigh-effort). Opt-in (Phase 37). |
| **oh-my-pi (omp)** | https://github.com/can1357/oh-my-pi | MIT | Can Bölük — a hard fork of Mario Zechner's `badlogic/pi-mono` (the same upstream as the stack's sandboxed phase-15 pi). Terminal AI coding agent, TUI-first: sessions, subagents, hash-anchored edits, LSP/DAP, extensions. Installed as a digest-verified prebuilt arm64 release binary (`omp/`) tracking the latest upstream release; run via `bin/omp` under the stack-owned `OMP_PROFILE=ai-stack`. All model calls route through LiteLLM on a scoped key (default `claude-opus-sub-xhigh`; upstream's `yolo` auto-approve disabled → `write`; update-check/auto-QA phone-home off). Opt-in (Phase 42). |
