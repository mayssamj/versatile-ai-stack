...
|---|---|---|
| `hermes_manager` | Chief of staff / operator / second brain — the single entrance: runs all of an EM's job (people, process, knowledge/memory, decisions, comms, triage) and turns intent into shipped reality in whatever shape it takes; executes directly when fastest, else delegates | claude-opus-sub-max |
...
| `hermes_qa_test_engineer` | QA / test engineer — test strategy + automation; the green-bar quality gate | claude-opus-sub-max |
...
| `hermes_sre_engineer` | SRE — reliability, IaC, observability, CI/CD, safe deploys; prod-credentialed | claude-opus-sub-max |
| `hermes_incident_manager` | Incident manager (read-only) — incident command + blameless postmortems | claude-opus-sub-max |
