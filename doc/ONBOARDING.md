...
bash vz-ai-stack.sh doctor            # 56 checks — target all green
```
...
stack doctor        # 56 health checks + per-check auto-fix offers (your first move when something's off)
...
(the 9 opt-in extras are excluded — see §5).
...
## 5. Opt-in extras (Phases 21–25, 27–30) — add only what you want
