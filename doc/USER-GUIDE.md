...
this specific combination of 41 services. Not a programming novice — no
...
- Token counts, latency (~3-8s on M4), model field (`gemma4:e4b`)
...
curl -s http://ollama:11434/api/generate \
  -d '{"model":"gemma4:e4b","prompt":"in one sentence: what is fine-tuning?","stream":false}' \
  | jq -r '.response'
```
...
```bash
# Health + state
bash ~/ai-stack/vz-ai-stack.sh doctor                 # 47 health checks + auto-fix offers
...
Doctor stays at 49/49 across every profile flip as long as the underlying services are healthy. If doctor drops, fix it before you do anything else.
